...
**Foto real de la fachada añadida (1 ago):** el equipo descargó una foto real de la fachada de Xocolata Pan y Café en Cali (letrero + terraza con sillas rojas) y pidió integrarla a la sección. Se agregó como `assets/xoc-panaderia.jpg`, en un grid de dos columnas junto al párrafo de apertura (`.xh-historia__grid`/`.xh-historia__img`, 4:3, bordes redondeados, en mobile la foto sube arriba del texto). Verificado en vivo en desktop y mobile.

**Despliegue a producción (1 ago):** hasta ahora todo el trabajo de la sesión se había verificado solo en el servidor local de `shopify theme dev` (127.0.0.1:9292). El equipo pidió pasar a la versión final en Shopify — se confirmó que el tema usado (`190111580448`, "Xocolata Premium (desarrollo)") es en realidad el **tema en vivo** de la tienda (`gjkkyq-ac.myshopify.com`), no un borrador. Se pidió confirmación explícita antes de publicar (el push a tema en vivo requiere el flag `--allow-live` y quedó bloqueado por el clasificador de seguridad hasta recibir esa confirmación). Confirmado, se ejecutó `shopify theme push --theme 190111580448 --allow-live` — subida exitosa. Verificado en vivo directamente en `gjkkyq-ac.myshopify.com` (no en el preview local): la sección "Nuestra Historia" con la foto real se ve igual que en desarrollo.

**Retoque de la foto real de la fachada con IA (1 ago):** el equipo pidió mejorar la foto de la fachada (cartel "SE ARRIENDA" del edificio vecino visible, cielo blanco quemado, tonos fríos que no calzan con la paleta vino/dorado del sitio). Primer intento de subir la foto a ChatGPT vía el navegador falló repetidamente (el navegador aislado no puede interactuar con el diálogo nativo de subida de archivos de Windows) — se cambió de herramienta a Higgsfield CLI (`nano_banana_pro`), que sí acepta la ruta del archivo local directamente sin pasar por el navegador. Dos pasadas de edición sobre la foto real (manteniendo composición, letrero, terraza y sillas exactamente iguales): (1) quitar el cartel de arriendo, corregir el cielo a tono cálido de atardecer, subir la temperatura de color general; (2) iluminar el interior de la panadería — vitrinas con luz cálida encendida, pan visible, resplandor cálido derramándose hacia la terraza. Ambas versiones se mostraron al equipo antes de aprobar. Reemplaza `assets/xoc-panaderia.jpg` (mismo nombre, sin cambios de código). Publicado directo al tema en vivo (`shopify theme push --allow-live --only assets/xoc-panaderia.jpg`) y verificado en `gjkkyq-ac.myshopify.com`.

**Corrección de los carteles pequeños del toldo (1 ago):** revisando la foto retocada en detalle, se notó que los 3 carteles pequeños del toldo ("Xocolata" repetido a lo largo de la fachada) habían quedado con el subtítulo ilegible/deformado — un artefacto típico de los modelos de IA al regenerar texto pequeño repetido. Dos intentos adicionales pidiéndole a la IA que "corrigiera" el texto no funcionaron (uno incluso deformó el nombre "Xocolata" en un cartel). Se resolvió con un enfoque distinto: en vez de pedirle a la IA que invente texto, se recortó el cartel grande (perfectamente legible) de la misma foto con un script en Python (PIL + numpy — segmentación por color con flood-fill para aislar el óvalo exacto, incluyendo relleno de huecos para no perder las letras), y se pegó ese mismo cartel real —escalado y rotado para coincidir con el tamaño y ángulo de cada cartel pequeño— sobre los 3 carteles ilegibles. Resultado: los 3 ahora dicen "Xocolata pan y café" con nitidez real, sin depender de que la IA "adivine" texto. Publicado al tema en vivo y verificado en `gjkkyq-ac.myshopify.com`.

**Imagen desactualizada en la tarjeta "Empaques y soporte" de Insumos (1 ago):** la tarjeta de empaques en la sección "Detrás de cada producto, insumos con estándar industrial" seguía usando `xoc-insumo-empaques.jpg`, un mockup viejo de dos bolsas (diseño dorado/marrón anterior, con la empanada mostrando textura hojaldrada incorrecta) — nunca se actualizó cuando se rediseñó el empaque real. Se corrigió apuntando la tarjeta a `xoc-bunuelo-empaque.jpg` (la foto real del empaque del buñuelo, ya generada y en uso en la página de producto), sin duplicar el archivo. Solo cambio de código en `xoc-home.liquid` (una línea, referencia de imagen). Publicado al tema en vivo y verificado en `gjkkyq-ac.myshopify.com`.

**Carrusel de la colección ahora navega a cada producto (1 ago):** las 4 tarjetas del carrusel ("La colección") eran solo decorativas — no llevaban a ninguna parte al hacer clic. Se convirtió cada `<article>` en un `<a href="/products/...">` apuntando a la página real de cada producto (Empanada de Cambray, Buñuelo Relleno, Almojábana Especial, Pandebono en Rosca), manteniendo la clase `.xh-ccard` y el atributo `data-ccard` para que el JS del carrusel siguiera funcionando igual. Se agregó `display:block`, `text-decoration:none` y `color:inherit` a `.xh-ccard` para que el enlace no rompiera el layout ni mostrara el subrayado azul por defecto. El carrusel ya tenía lógica de arrastre-vs-clic en `xoc-animations.js` (`if (moved) { e.preventDefault(); e.stopPropagation(); }`), así que no hizo falta tocar el JS — un clic real navega, un arrastre para deslizar entre tarjetas no dispara la navegación. Verificado en vivo: clic navega a la página del producto, arrastre solo desliza el carrusel, probado en desktop y mobile.
...
### Fallback de autoplay para los videos en móvil (3 ago)
El equipo reportó que en algunos teléfonos, en vez de reproducirse el cinemagraph del pandebono (o del hero) automáticamente, aparece la foto fija con un botón de play — como si el video estuviera roto. Diagnóstico: el markup de ambos videos (`.xh-hero__video`, `.xh-annotated__video`) ya tenía los atributos correctos (`autoplay muted loop playsinline`), pero algunos navegadores móviles (modo "Ahorro de datos"/Lite de Chrome en Android, modo de bajo consumo, ciertos navegadores de fabricante) bloquean el autoplay de video como política deliberada del sistema, sin importar los atributos — en vez de reproducir, muestran su propia UI nativa de poster + botón de play, que se ve rota porque nunca se pensó como reproductor manual.

**Corregido:** se agregó una salvaguarda en `xoc-animations.js` (fuera del bloque que depende de GSAP, corriendo siempre) que intenta reproducir cada `video[autoplay]` explícitamente vía JS y, si el navegador lo bloquea (la promesa de `.play()` es rechazada, o el video sigue en pausa 2 segundos después por bloqueo silencioso como el modo Ahorro de Datos), oculta el elemento `<video>` — dejando ver la foto de fondo (`background-image`) que cada contenedor ya tenía como respaldo para `prefers-reduced-motion`. El resultado es una foto fija limpia e intencional en vez de un botón de play que parece roto. Verificado en vivo que ambos videos (hero y nostalgia) siguen reproduciéndose con normalidad en un navegador que sí permite autoplay — no hay regresión.

### Corrección: años de trayectoria en "Nuestra Historia" (3 ago)
El equipo confirmó directamente que la trayectoria real es de **30 años**, no 11. El "11 años" que se usó al construir la sección venía de una fuente indirecta (la biografía de Instagram de la panadería, "11 años llevando pan a los hogares caleños"), citada como tal en su momento — el dato directo del equipo prevalece sobre esa investigación web. Corregido en `xoc-home.liquid`: el titular ("30 años sirviendo pan..."), la franja de dato verificado ("30 AÑOS EN CALI") y el cierre del párrafo ("se hornea todos los días, desde hace 30 años" — antes decía "más de una década"), en ambos idiomas. Publicado al tema en vivo y verificado en `gjkkyq-ac.myshopify.com`.

### Correcciones de precisión de producto y copy comercial (3 ago)
El equipo detectó varias inexactitudes revisando la web contra el empaque y panel nutricional reales, y pidió corregirlas de una vez:

- **Peso de la Empanada de Cambray: 60 g → 80 g.** El empaque real impreso dice 80 g; la web (carrusel, stat card de "Cómo viene tu pedido" y la porción del panel nutricional en la página de producto) tenía 60 g por error. Corregido en `xoc-home.liquid` y `xoc-product-empanada.liquid`, ambos idiomas. Los valores nutricionales (240 cal, 11 g grasa, 190 mg sodio, 29 g carbohidratos, 6 g proteína) ya correspondían a la porción de 80 g del panel real — solo hacía falta corregir la etiqueta de la porción, no los números.
- **Relleno del Buñuelo: "arequipe o queso" → "arequipe y queso pasteurizado".** El equipo confirmó que el relleno real lleva ambos, no uno u otro. Corregido en el tagline de `xoc-product-bunuelo.liquid` y en la descripción del carrusel en `xoc-home.liquid`. El panel de ingredientes detallado del Buñuelo ya listaba correctamente los quesos como pasteurizados — no se tocó.
- **Lenguaje de "queso maduro" → "queso pasteurizado" en copy de marketing.** "Queso maduro" (Almojábana, tagline y carrusel) sonaba a queso artesanal sin pasteurizar, lo cual no es preciso ni conveniente para un mercado en EE. UU. Corregido en `xoc-home.liquid` y `xoc-product-almojabana.liquid`. **No se tocó** el panel de ingredientes real de la Almojábana (`harina de trigo`, `queso pasteurizado`, `queso campesino pasteurizado`), que ya es correcto y viene del panel nutricional real del empaque — ver corrección de [portafolio.md](../02-producto/portafolio.md) más abajo.
- **Corrección de alérgenos en `portafolio.md`:** el documento decía que solo la Empanada de Cambray llevaba harina de trigo; es al revés — la Empanada usa harina de maíz (sin gluten) y es la **Almojábana Especial** la que sí lleva harina de trigo, confirmado por el panel nutricional real leído directamente del empaque (ver entrada del 31 jul más arriba). Corregido para que ambos documentos sean consistentes.
- **Reescritura de la tabla comparativa "No vendemos congelados. Construimos aliados."** Los 4 ítems originales (ej. "Cero personal especializado, cero merma, food-cost predecible") sonaban genéricos, como relleno. Se reescribieron para ser más concretos y persuasivos desde la perspectiva del dueño del negocio: el ítem débil se reemplazó por la Garantía Triple Sello (reposición total si falla la cadena de frío, degustaciones sin costo si no rota en 45 días) — un diferencial real que antes no aparecía en esta tabla — y los otros 3 se afilaron para sonar más específicos y menos corporativos. Corregido en `xoc-home.liquid`, ambos idiomas.
...
### Correo unificado y "Nuestra Historia" a nivel nacional (4 ago)
Dos correcciones más pedidas por el equipo:

- **Correo de contacto unificado.** El sitio tenía dos correos distintos en distintos puntos: `comercial@xocolatafoodpremium.com` (footer y las 4 páginas de producto) y `xocolatapremium@gmail.com` (ya estaba puesto a mano en el home vía el editor de temas). Antes de tocar nada se hizo `theme pull` de los templates/secciones afectadas para no pisar ese cambio ya hecho en el editor. Se dejó `xocolatapremium@gmail.com` como el único correo en todos los puntos de contacto: los 4 `cta_url` de las páginas de producto (`templates/product.*.json`), el `cta_url` del footer (`sections/footer-group.json`), el enlace visible del footer y la línea de contacto del cierre en el home (`xoc-home.liquid`), y los valores por defecto en el código de las 6 secciones (`xoc-home`, `xoc-footer`, y los 4 `xoc-product-*`) para que cualquier instancia nueva use el correo correcto sin depender de que alguien lo configure a mano.
- **"Nuestra Historia" — de Cali a nivel nacional.** La sección estaba redactada como si Xocolata operara solo en Cali (títulos y menciones a barrios específicos: Capri, Bochalema, Autopista, Guabal; badges "30 AÑOS EN CALI" y "VARIOS PUNTOS EN LA CIUDAD"). El equipo aclaró que la operación es a nivel nacional, no local de una ciudad — se reescribió el titular ("30 años llevando pan a los hogares de Colombia"), el párrafo de apertura (sin nombrar barrios, hablando de "familias colombianas" en vez de "familias caleñas"), los dos badges de dato verificado ("30 AÑOS DE TRAYECTORIA" y "PRESENCIA A NIVEL NACIONAL") y el cierre de la sección ("De la vitrina de un barrio en Colombia..."), en ambos idiomas. No se tocó la dirección del footer (`Cali, Colombia`), que es un dato de sede/contacto, no parte del relato de marca.
